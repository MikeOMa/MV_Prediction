from core_functions import data, read_models, models_list, indep_models_list, folds, min_max_scaler
from probdrift import X_VAR, Y_VAR, mpl_config
from probdrift.MVN_helpers import elipse_points, in_ci
import pyproj
import matplotlib.pyplot as plt
import cartopy.crs as ccrs
cproj = ccrs.TransverseMercator(central_longitude=-78, central_latitude=28, approx=False)
pyprojection = pyproj.Proj(**cproj.proj4_params)

projection = pyproj.Proj(proj="tmerc", lat_0=28, lon_0=-78, k=1.00000, x_0=0, y_0=0,
                         towgs84=[-90.7, -106.1, -119.2, 4.09, 0.218, -1.05, 1.37], units="km")
...
def arrow_vel_proj(projection, df_row, scipy_dist, ax, scale=1):
    ...
    ax.arrow(*pos, *u, label="Truth", color='black', head_width=0.15, length_includes_head=True)
    u_pred = scipy_dist.mean * 1e-2 * scale
    ax.arrow(*pos, *u_pred, label="Mean Prediction", color='red', head_width=0.15, length_includes_head=True,
...
def plot_elipse_proj(projection, df_row, scipy_dist, ax, scale=1):
    ...
    ax.plot(x + scipy_dist.mean[0] * scale * 1e-2 + points[:, 0], y + scipy_dist.mean[1] * scale * 1e-2 + points[:, 1],
            color='red',
            linestyle="--", label='70\% PR')
import string
def add_letters(axs):
    for n, ax in enumerate(axs.flatten()):
        ax.text(-0.15, 0.95, "\\textbf{" + string.ascii_uppercase[n] + ")}", transform=ax.transAxes,
                size=13, weight='bold')
...
def add_xy_grid_lines(ax):
    ...
    gl.xlines = True
    gl.ylines = True
    gl.xformatter = mticker.FuncFormatter(lambda x,u: nice_km_format(pyprojection.transform(x, min(y_ticks_in_ll))[0]))
    gl.yformatter = mticker.FuncFormatter(lambda x,u: nice_km_format(pyprojection.transform(min(x_ticks_in_ll), x)[1]))
    return gl
...
def plot_traj_preds_northing(df_to_plot, models_list, ax):
    ...
    ax.plot(df_to_plot["lon"], df_to_plot["lat"], "b-o", alpha=0.1, transform=ccrs.PlateCarree())
    ax.coastlines()
    pred_distns = models_list[0].scipy_distribution(min_max_scaler.transform(df_to_ellipse[X_VAR]))
    ...
    for (i, df_row), scipy_dist in zip(df_to_ellipse.iterrows(), pred_distns):
        ...
        ax.arrow(*pos, *u, label="Truth", color='black', head_width=1e4, length_includes_head=True)
        u_pred = scipy_dist.mean * 1e-2 * scale
        ax.arrow(*pos, *u_pred, label="Mean Prediction", color='red', head_width=1e4, length_includes_head=True)
    ...
    ax.arrow(*xy_for_scale_arrow, scale, scale, color='C2', label="(1,1)m/s scaled to km/day", head_width=1e4)
